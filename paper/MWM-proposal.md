...
**Third, the interpretability frontier moves.** If the representation was built to be read from the start, reading it is not a research program — it is a `SELECT` statement. The belief-state, the evidence chains, the values, the recent updates — all are inspectable as graph structure. What does *not* become a `SELECT` statement is the LLM's forward pass that turns a rendered subgraph into a sentence. That step is still a transformer doing transformer things. MWM's contribution at the interpretability layer is therefore precise: it makes *retrieval* interpretable by construction, and it leaves the *rendering* step opaque — but over a smaller, semantically clean, structured input. The original problem ("why did this output appear?") is decomposed into a part the architecture solves natively and a part that still needs interpretability research, but on a much more tractable surface than arbitrary natural language. The right framing is "interpretable retrieval, with reduced rendering opacity," not "interpretability solved."

This is not a proposal to replace pretrained weights. It is a proposal to *redistribute* the work they currently do. Language stays in the LLM. Knowledge moves to the graph. Reasoning moves to a symbolic/numeric tool. Each component carries the load it is suited for. The bet is that this redistribution makes the agent's beliefs auditable and correctable; the cost story for that bet is engaged with explicitly in Section 6.
...
The graph supports multiple edge *types*, and each does different work. Hebbian co-occurrence edges form by experience and provide the substrate of association — they are necessary but, on their own, would carry the same structural limits PMI/LSA/HAL hit historically. On top of that substrate, the architecture posits three additional first-class edge types that do most of the load-bearing work for higher-order reasoning. **Causal edges** carry direction and Pearl Layer 2 semantics — they are inferred from interventional episodes during consolidation (see §4.2) and let the agent reason about counterfactuals and forward-model the consequences of action. **Affordance edges** connect objects/contexts to action-possibilities, grounding the agent's pragmatic understanding in what can be *done* with a thing rather than only what it co-occurs with. **Second-order/analogical edges** connect *edges to edges*, supporting reasoning about relations between relations (the structure-mapping move at the heart of analogy). Co-occurrence is the substrate; causal, affordance, and second-order edges are the structure that makes the substrate carry meaning. Treating co-occurrence as the only association mechanism is exactly the move that produces a brittle, superstition-prone graph; the architecture explicitly rejects that reduction. Reconsolidation (every traversal is a write) further protects the system: superstitious or accidentally-formed associations decay if they are never reinforced through use, while load-bearing beliefs are repeatedly strengthened.

A note on pretrained embeddings: the architecture does not reject them. Off-the-shelf embeddings continue to play a real role in *novel-edge formation* (computing similarity between newly-encoded nodes when no experiential edge yet exists) and *modality-internal neighborhood lookup* (finding candidate neighbors during tree growth before experiential edges have had time to form). What pretrained embeddings do *not* do is carry the agent's canonical belief structure — that lives in the graph topology, not the latent space. The relationship is one of demotion, not rejection: embeddings support graph construction; the graph carries the world-model.
...
### 5.4 Structural interpretability of retrieval

**Prediction (what graph-only audits *can* answer).** A researcher with no access to the LLM's weights, given only the Matrix, can reconstruct (a) the agent's current beliefs about a specified domain, (b) the evidence chain supporting any specified belief, (c) the agent's current values, and (d) recent changes to its belief-set, without any model inference. These are properties of graph topology and node content; reading them is a query, not a probe.

**Out of scope (what graph-only audits *cannot* answer).** A graph-only audit cannot fully explain why the LLM phrased its response one way rather than another given the same rendered subgraph. The forward pass that turns the Tree into a sentence is still a transformer, and surface-form choices, register, framing, and disambiguation that happen at rendering time remain on the wrong side of the interpretability frontier. The claim of this prediction is bounded: the *retrieval and belief-state* are interpretable by construction; the *rendering layer* is not.

**Why this is still a meaningful win.** The rendering step happens over a much smaller and more structured input than the agent's full knowledge. Interpretability techniques (sparse autoencoders, probes, circuit-finding) have far more traction over a small, semantically clean context than over the open distribution of natural language. MWM does not solve rendering opacity; it shrinks the surface that opacity sits on, while making everything upstream of rendering auditable.

**Measurement.** Blind-audit protocol. Researcher is given a Matrix snapshot and a set of questions. Researcher must answer (a)–(d) without querying the LLM. Compare answers to the agent's behavior under the same questions. High agreement rate on (a)–(d) supports the bounded interpretability claim. A separate sub-experiment can characterize the gap: cases where the agent's behavior matches what the graph predicts (interpretability holds) versus cases where rendering choices materially shift the behavior (rendering opacity is doing work the graph audit missed).

**Failure mode.** If graph-only audits disagree systematically with runtime behavior on (a)–(d), the LLM is inferring beyond what the graph supports, which means the graph is not the primary belief substrate. That would falsify the thesis. Disagreement on rendering-style questions (tone, phrasing, framing) is *expected* and is not a falsifier — it is the bounded scope of the claim.
...
The framing that matters more than per-token throughput is *end-to-end task cost*. For tasks where current LLM pipelines need heavy downstream verification — fact-checking, source-tracing, hallucination correction, retries inside agentic loops, human-in-the-loop validation for high-stakes decisions — verification dominates the cost picture. MWM moves verification *into* the architecture: the path is the trace, the trace is the audit, and the agent does not produce claims whose provenance is not already structural. Per-token slower; per-trustworthy-task possibly comparable. That comparison itself is one of the things the implementation phase is set up to measure. The architecture is not defensible for tasks where raw throughput is the binding constraint; it is targeted at tasks where the bottleneck is *trust*.
...
### 7.3 Neurosymbolic memory and knowledge graphs
...
### 9.3 The open research questions
...
The architecture is well-specified enough to build. The questions below fall into two groups: ones with partial answers in the project's research archive, and ones that are genuinely open. The research archive — six rounds of multi-stream subagent investigation, plus design documents and dedicated research notes — is where the load-bearing engineering work has been worked out so far. Where a question has been partially addressed there, this section points forward; where it is genuinely open, this section names that. The full archive lives in [`research/`](../research/) and [`design/`](../design/).

**Memory encoding — what becomes a node?** *Partial answer in [`design/architecture.md`](../design/architecture.md) and [`research/round4-synthesis.md`](../research/round4-synthesis.md).* The intake valve of the entire system. Every message? Every topic shift? Every "significant" moment? Who decides — the LLM, a heuristic, a surprise-gating model? Current working answer: a hybrid — the subconscious Tree encodes aggressively from any new material, consolidation later decides what to keep. But the granularity of encoding is the parameter most likely to determine whether the graph grows useful structure or noise; concrete empirical work is required.

**The LLM/graph boundary during reasoning.** *Partial answer in [`research/local-cognition/`](../research/local-cognition/).* The vision says the LLM reasons *from* the graph, never from its pretrained weights alone. But some questions — math, logic, multi-step deduction — genuinely need inference beyond what the graph holds. The local-cognition research note sketches a working agent that uses a small local LLM for traversal, the graph for belief structure, and tool calls for math and logic, with explicit boundaries between in-LLM inference and tool-routed reasoning. The boundary placement remains an empirical question, but the architectural shape is sketched.

**Causal discovery parameters.** *Partial answer in [`research/round5-synthesis.md`](../research/round5-synthesis.md) and [`design/open-questions.md`](../design/open-questions.md).* Minimum sample thresholds for GFCI. Handling of unknown intervention targets. Bayesian vs. frequentist approach. LLM-prior integration without contamination. Round 5 introduced the causal-discovery pipeline; the parameter sweeps and the comparison against Bayesian alternatives are concrete empirical work to run on the working prototype.

**Scaling, sharding, and the cost frontier.** *Cost picture engaged with in Section 6; hardware direction in [`research/compute-architecture/notes.md`](../research/compute-architecture/notes.md).* How big can the graph get before traversal breaks? When does SQLite need to be replaced? Can graphs shard (shared common-knowledge subgraph + per-agent personal graph)? Section 6 above names the working-regime versus asymptotic distinction; the compute-architecture research direction sketches the longer-horizon move toward graph-tuned substrate. Multi-agent amortization (shared common-knowledge subgraphs) is genuinely open.

**Value drift vs. value stability.** *Genuinely open.* The same mechanism that lets the agent learn — reconsolidation — lets values drift. Should some nodes be immutable? If so, which? Who decides? This is an alignment-adjacent question with no clean answer, and it is one of the questions the implementation phase is set up to surface concretely.

**Control precedence in the Mirror.** *Surfaced in [`research/round6-synthesis.md`](../research/round6-synthesis.md); genuinely open.* When competing modulators fire — urgency says act, caution says wait — which wins? Simple argmax over intensities? Value-alignment-weighted? Learned from experience? Round 6 surfaced this through the action-readiness work; it remains open.

**The values/emotions/Mirror layer as engineering specification.** *Partial answer in [`research/round6-synthesis.md`](../research/round6-synthesis.md) (interoceptive predictor stream and action-readiness stream).* A reviewer reasonably asks: what is the formal specification — input, output, update rule — of an emotion in this system? Round 6's interoceptive-predictor work and action-readiness model are concrete enough to engineer; the formalization needs to be lifted out of the research synthesis and into the implementation specification. This is staged for the post-prototype phase.

**Multimodal substrate details.** *Elevated to Section 9; full articulation in [`design/multimodal-unified-substrate.md`](../design/multimodal-unified-substrate.md); candidate substrate developed in the sibling [ULP project](../../ULP/).* Text-layer density, who writes and updates descriptions, cross-modal traversal weighting, extensibility. This is the research program underneath the commitment to a unified substrate. §9.4 introduces ULP as the specific candidate substrate under active investigation; the ULP-MWM coupling is staged, not yet load-bearing on the prototype.

**The planning horizon.** *Genuinely open.* The Mirror handles immediate evaluation well. Future-directed intentions — what the agent is doing in a week, a month — are not yet specified. How does a multi-session plan persist and constrain present Trees? This is one of the questions the implementation phase will surface.

**Evaluation against Lindsey-style emergent emotion.** *Predicted in §5.3; experimental work pending.* The comparison itself has not yet been run; it is set up as one of the structured evaluations once the prototype exists.

**Gödelian limits.** *Genuinely open.* The Mirror cannot verify its own completeness (Gödel); observation changes the observed (measurement). These are mathematically real. How they manifest in practice — and how systematic blind spots get detected — is an open theoretical and empirical question.
