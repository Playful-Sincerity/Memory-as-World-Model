...
**Nodes** carry: content (a piece of text, an image pointer, a sensor reading, a multimodal blob), confidence (how much the agent currently trusts this), evidence (which other nodes support or are supported by this one), timestamps (when first encoded, when last traversed), and a priority weight that governs resistance to pruning and decay.
...
**Multimodal integration.** Language is the first modality. Images, audio, sensor data come next. How are they embedded? How do they connect to text nodes? Is a unified multimodal embedding space (SONAR-style) the right substrate, or should modalities stay separate and connect only through text descriptions?
